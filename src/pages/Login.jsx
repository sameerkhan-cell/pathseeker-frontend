...
export default function Login() {
  ...
  const adminCreds = { email: "admin@pathseeker.com", pass: "AdminPass123" };
    ...
    <div className="min-h-screen w-full flex flex-col lg:flex-row bg-[#0D1117] text-white selection:bg-accent-gold selection:text-[#14151A]">
      ...
      <div className="lg:w-1/2 w-full bg-[#FAF1EC] dark:bg-[#14151A] text-[#14151A] dark:text-white p-6 sm:p-12 flex items-center justify-center relative theme-transition">
        <div className="w-full max-w-md bg-white dark:bg-[#1B212D] border border-[#E5DCD0] dark:border-white/10 rounded-2xl shadow-xl p-8 sm:p-10 space-y-6 theme-transition">
          ...
          {/* Quick Demo 2-Role Grid (Student + Professional) */}
          ...
          {/* Admin Access — Separate Collapsible Section */}
          <div className="rounded-xl border border-amber-500/20 bg-amber-50/60 dark:bg-amber-500/5 overflow-hidden">
            <button
              type="button"
              onClick={() => setShowAdminCreds(!showAdminCreds)}
              className="w-full flex items-center justify-between px-4 py-2.5 text-xs font-semibold text-amber-700 dark:text-amber-400 hover:bg-amber-100/60 dark:hover:bg-amber-500/10 transition-colors cursor-pointer"
            >
              <span className="flex items-center gap-2">
                <ShieldCheck className="w-3.5 h-3.5" />
                Admin Portal Access
              </span>
              <span className="text-[10px] font-mono opacity-60">{showAdminCreds ? "▲ hide" : "▼ show"}</span>
            </button>
            {showAdminCreds && (
              <div className="px-4 pb-3 space-y-1.5 border-t border-amber-500/15">
                <div className="flex items-center justify-between pt-2">
                  <span className="text-[11px] text-amber-700/70 dark:text-amber-400/70 font-mono">Email</span>
                  <button
                    type="button"
                    onClick={() => setForm({ email: adminCreds.email, password: adminCreds.pass })}
                    className="text-[11px] font-mono text-amber-700 dark:text-amber-300 hover:text-accent-gold cursor-pointer"
                  >
                    {adminCreds.email}
                  </button>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-[11px] text-amber-700/70 dark:text-amber-400/70 font-mono">Password</span>
                  <span className="text-[11px] font-mono text-amber-700 dark:text-amber-300">{adminCreds.pass}</span>
                </div>
                <button
                  type="button"
                  onClick={() => setForm({ email: adminCreds.email, password: adminCreds.pass })}
                  className="w-full mt-1 py-1.5 rounded-lg text-[11px] font-semibold font-mono bg-amber-500/15 hover:bg-amber-500/30 text-amber-700 dark:text-amber-300 border border-amber-500/20 transition-colors cursor-pointer"
                >
